...
import { prefetchSeriesExperience } from '@/lib/route-prefetch';
...
const getSeriesLanguageHintScore = (hint?: { tone: 'strong' | 'subtitle' | 'weak'; label: string } | null) => {
    if (!hint) return 0;
    if (hint.tone === 'strong') return 3;
    if (hint.tone === 'subtitle') return 2;
    return 1;
};
...
// ==========================================
const SeriesCard: React.FC<{ series: Series; index: number; languageHint?: { tone: 'strong' | 'subtitle' | 'weak'; label: string; detail?: string } | null }> = ({ series, index, languageHint }) => {
    ...
    const detailsPath = `/series/${series.id}`;
    const handlePrefetch = (priority: 'immediate' | 'idle' = 'immediate') => prefetchSeriesExperience(series.id, priority);
                    ...
                    {languageHint && (
                        <div className="pt-1">
                            <div className="space-y-1">
                                <span className={`inline-flex items-center rounded-full px-2 py-1 text-[9px] font-bold uppercase tracking-wide ${
        ...
        <AnimatePresence>
            {open && (
                <motion.div
                    ...
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
                    className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-xl p-3 sm:p-6"
                    onClick={onClose}
                >
                    <motion.div
                        ...
                        className="glass-card rounded-[1.5rem] sm:rounded-3xl p-4 sm:p-8 max-w-lg w-full space-y-5 sm:space-y-6 max-h-[92vh] overflow-y-auto"
                    >
                        <div className="flex items-center gap-3">
                            <div className="p-2.5 bg-primary/10 rounded-xl border border-primary/20">
                                <Plus className="w-5 h-5 text-primary" />
                            </div>
                            <h2 className="text-lg sm:text-xl font-bold text-white">Adicionar Série</h2>
                        </div>
...
export const SeriesPage: React.FC = () => {
    ...
    const portugueseForYouSeries = React.useMemo(() => {
        return [...filteredSeries]
            ...
            .sort((a, b) => {
                ...
                const hintDelta = getSeriesLanguageHintScore(getPtbrCoverageHint(metaB)) - getSeriesLanguageHintScore(getPtbrCoverageHint(metaA));
                if (hintDelta !== 0) return hintDelta;
                const ptbrDelta = Number(metaB?.ptbrConfidence || 0) - Number(metaA?.ptbrConfidence || 0);
                if (ptbrDelta !== 0) return ptbrDelta;
                return getSeriesDiscoveryReadiness(metaB) - getSeriesDiscoveryReadiness(metaA);
            })
            .slice(0, 8);
    }, [filteredSeries, discoverySeriesMeta]);

    const rankedFilteredSeries = React.useMemo(() => {
        return [...filteredSeries].sort((a, b) => {
            ...
            const readinessDelta = getSeriesDiscoveryReadiness(metaB) - getSeriesDiscoveryReadiness(metaA);
            if (readinessDelta !== 0) return readinessDelta;

            return (Number(b.progress) || 0) - (Number(a.progress) || 0);
        });
    }, [discoverySeriesMeta, filteredSeries]);

    if (loading) {
        return (
            <div className="min-h-screen bg-background pt-24 px-4 sm:px-6 pb-20">
                <ExperienceLoader label="Organizando catalogo de series" variant="catalog" className="px-0" />
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen bg-background pt-24 px-4 sm:px-6 pb-20">
            <div className="max-w-7xl mx-auto space-y-7 sm:space-y-8">
                {/* Header */}
                <motion.div
                ...
                >
                    <div className="flex items-start sm:items-center gap-4">
                        <div className="p-3 sm:p-4 bg-gradient-to-br from-purple-500/10 to-primary/10 rounded-2xl border border-purple-500/20">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center gap-3 w-full md:w-auto">
                        ...
                        <div className="relative w-full sm:w-auto">
                            ...
                            <input
                                ...
                                className="w-full sm:w-64 pl-10 pr-4 py-3 rounded-xl bg-white/[0.03] border border-white/10 text-white placeholder-white/20 outline-none focus:border-primary/50 transition-colors text-sm"
                            />
                        </div>
                        ...
                        <button
                            ...
                            className="flex items-center justify-center gap-2 px-5 py-3 bg-primary text-white font-semibold rounded-xl hover:bg-primary/90 transition-colors shadow-lg shadow-primary/20 w-full sm:w-auto"
                        >
                ...
                {portugueseForYouSeries.length > 0 && !searchQuery && (
                    <div className="space-y-5">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h2 className="text-xl sm:text-2xl font-black text-white tracking-tight uppercase italic leading-tight">
                                    Series em portugues para voce
                                </h2>
                                <p className="text-[9px] sm:text-[10px] text-cyan-200/70 font-black uppercase tracking-[0.22em] sm:tracking-[0.35em] mt-1">
                                    Priorizadas pelo Arconte para a casa
                                </p>
                            </div>
                        </div>

                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-8 gap-2.5 sm:gap-4">
                            {portugueseForYouSeries.map((s, idx) => (
                                <SeriesCard key={`pt-${s.id}`} series={s} index={idx} languageHint={getPtbrCoverageHint(discoverySeriesMeta.get(s.id))} />
                            ))}
                        </div>
                    </div>
                )}

                {rankedFilteredSeries.length > 0 ? (
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-2.5 sm:gap-4 md:gap-5">
                        {rankedFilteredSeries.map((s, idx) => (
                            <SeriesCard key={s.id} series={s} index={idx} languageHint={getPtbrCoverageHint(discoverySeriesMeta.get(s.id))} />
                        ))}
                    </div>
                ) : (
                    <motion.div
