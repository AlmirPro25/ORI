...
import { BACKEND_URL, GATEWAY_URL, NEXUS_URL } from '@/lib/endpoints';

const NEXUS_API = NEXUS_URL;
...
export function TorrentSearch() {
    ...
    const handleAddToCatalog = async (result: TorrentResult) => {
        try {
            await axios.post(`${BACKEND_URL}/api/v1/videos/auto-ingest`, {
                title: result.title,
    ...
    const handleWatchNow = async (result: TorrentResult) => {
        ...
        try {
            axios.post(`${BACKEND_URL}/api/v1/videos/auto-ingest`, {
        ...
        <div className="min-h-screen bg-background text-white relative overflow-hidden flex flex-col pt-24">
            ...
            <div className="container mx-auto px-4 sm:px-6 relative z-10 max-w-6xl">
                ...
                <motion.div
                    ...
                    className="text-center space-y-4 sm:space-y-6 mb-10 md:mb-16"
                >
                    ...
                    <h1 className="text-4xl sm:text-5xl md:text-7xl font-black uppercase italic tracking-tighter leading-none mb-2">
                        Nexus <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-purple-500">Deep Search</span>
                    </h1>
                    <p className="text-base sm:text-lg md:text-xl text-white/40 max-w-2xl mx-auto font-medium">
                ...
                <motion.div
                    ...
                    className="bg-black/40 backdrop-blur-2xl border border-white/10 rounded-[1.5rem] sm:rounded-[2rem] p-4 sm:p-6 md:p-8 shadow-2xl relative overflow-hidden group mb-10 md:mb-12"
                >
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row gap-4 mb-8">
                        <div className="flex-1 relative group/input">
                            ...
                            <Input
                                ...
                                className="pl-14 sm:pl-16 h-14 sm:h-16 md:h-20 bg-white/5 border-white/10 rounded-2xl text-base sm:text-lg md:text-xl text-white placeholder:text-white/20 focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500/50 transition-all shadow-inner"
                            />
                        </div>
                        <Button
                            onClick={handleSearch}
                            disabled={loading || !query.trim()}
                            className="h-14 sm:h-16 md:h-20 px-6 sm:px-8 md:px-12 rounded-2xl bg-white text-black font-black uppercase tracking-[0.2em] md:tracking-widest text-sm sm:text-base md:text-lg hover:bg-cyan-400 transition-all shadow-glow hover:scale-105 active:scale-95 disabled:opacity-50 disabled:hover:scale-100 w-full md:w-auto"
                        >
                    ...
                    {/* Filters & Mode */}
                    <div className="flex flex-col md:flex-row justify-between items-stretch md:items-center gap-4 md:gap-6 relative z-10">
                        <div className="flex bg-white/5 p-1.5 rounded-xl border border-white/10 overflow-x-auto no-scrollbar">
                            {(['All', 'Movies', 'TV', 'Anime'] as const).map((cat) => (
                                <button
                                    ...
                                    className={cn(
                                        "px-4 sm:px-6 py-2.5 rounded-lg text-xs font-black uppercase tracking-wider transition-all whitespace-nowrap",
                                        category === cat
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 bg-white/5 px-4 py-3 sm:py-2 rounded-xl border border-white/10">
                            ...
                            <div className="flex gap-2 overflow-x-auto no-scrollbar">
                                {(['semantic', 'ptbr', 'ultra', 'extended', 'advanced'] as const).map((mode) => (
                                    <button
                                        ...
                                        className={cn(
                                            "px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider transition-all whitespace-nowrap",
                                            searchMode === mode
                                                ? mode === 'semantic'
