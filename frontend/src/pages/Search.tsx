...
export const SearchPage: React.FC = () => {
    ...
    if (searching) {
        return (
            <div className="min-h-screen bg-background pt-28 md:pt-32 pb-20 px-4 sm:px-6 md:px-12">
                <ExperienceLoader label="Sincronizando busca global" variant="search" className="px-0" />
    ...
    const getSearchPtbrBonus = React.useCallback((torrent: any) => {
        let bonus = 0;
        if (torrent.hasPTBRAudio) bonus += 12;
        if (torrent.hasPTBRSubs) bonus += 5;
        if (torrent.ptbrScore) bonus += Number(torrent.ptbrScore) * 0.3;
        if (torrent.isCompletePack) bonus += 2;
        if (torrent.isSeasonPack) bonus += 1;
        return bonus;
    }, []);

    const filteredSeriesBase = selectedSeason
        ? (globalResults.series || []).filter((t: any) => t.detectedSeason === selectedSeason || t.isCompletePack)
        : (globalResults.series || []);
    const filteredSeries = [...filteredSeriesBase].sort((a: any, b: any) => {
        const bonusA = getSearchPtbrBonus(a);
        const bonusB = getSearchPtbrBonus(b);
        if (bonusA !== bonusB) return bonusB - bonusA;
        return Number(b.ptbrScore || b.seeds || 0) - Number(a.ptbrScore || a.seeds || 0);
    });
    ...
    const renderTorrentBadges = (torrent: any) => {
        const searchPtbrBonus = getSearchPtbrBonus(torrent);
        return (
        <div className="flex items-center gap-1.5 flex-wrap">
            ...
            {searchPtbrBonus >= 12 && (
                <span className="text-[7px] font-black px-1.5 py-0.5 bg-emerald-500/15 text-emerald-300 rounded uppercase border border-emerald-400/20">PT-BR forte</span>
            )}
            {searchPtbrBonus >= 5 && searchPtbrBonus < 12 && (
                <span className="text-[7px] font-black px-1.5 py-0.5 bg-cyan-500/15 text-cyan-300 rounded uppercase border border-cyan-400/20">Mais legenda</span>
            )}
        </div>
    )};
    ...
    const renderTorrentCard = (torrent: any, index: number, colorAccent: string = 'green') => {
        ...
        return (
            <motion.div
                ...
                className={`group cursor-pointer bg-white/[0.03] border border-white/[0.06] rounded-2xl p-4 sm:p-5 hover:border-${colorAccent}-500/50 transition-all hover:bg-white/[0.06] relative overflow-hidden w-[18rem] sm:w-72 md:w-auto flex-shrink-0 md:flex-shrink`}
    ...
    return (
        <div className="min-h-screen bg-background pt-28 md:pt-32 pb-20 px-4 sm:px-6 md:px-12 relative overflow-hidden">
            <AddonStreamDialog
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <motion.div
                    initial={{ opacity: 0, y: -20 }}
                    animate={{ opacity: 1, y: 0 }}
                    className="mb-8 sm:mb-10 md:mb-12 bg-black/40 backdrop-blur-2xl border border-white/10 rounded-2xl md:rounded-3xl p-4 sm:p-6 shadow-2xl"
                >
                    <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                        <div className="flex-1 relative group/input">
                            ...
                            <Input
                                ...
                                className="pl-14 h-12 sm:h-14 bg-white/5 border-white/10 rounded-2xl text-base sm:text-lg text-white placeholder:text-white/20 focus:ring-2 focus:ring-primary/50 focus:border-primary/50 transition-all"
                            />
                        </div>
                        <Button
                            onClick={handleSearch}
                            disabled={searching || !searchInput.trim()}
                            className="h-12 sm:h-14 px-6 sm:px-8 rounded-2xl bg-primary text-black font-black uppercase tracking-wider hover:bg-primary/90 transition-all disabled:opacity-50 w-full sm:w-auto"
                        >
                ...
                <header className="mb-10 sm:mb-12 md:mb-16 space-y-5 sm:space-y-6 border-b border-white/5 pb-8 md:pb-12">
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 sm:gap-8">
                        <div className="space-y-4">
                            ...
                            <h1 className="text-2xl sm:text-4xl md:text-7xl font-black tracking-[-0.04em] md:tracking-[-0.05em] uppercase leading-[0.92] text-white break-words">
                        ...
                        <div className="text-left md:text-right space-y-3">
                            <div className="text-3xl sm:text-4xl md:text-6xl font-black text-primary italic tracking-tight leading-none">
                                {totalEntities}
                            </div>
                    ...
                    <div className="space-y-20">
                        ...
                        {globalResults.seriesMetadata && (
                            <section>
                                ...
                                <div className="relative bg-white/[0.03] border border-white/[0.06] rounded-[1.5rem] sm:rounded-3xl overflow-hidden">
                                    ...
                                    <div className="relative p-4 sm:p-8 flex flex-col md:flex-row gap-4 sm:gap-8">
                                        {/* Poster */}
                                        ...
                                        <div className="flex-1 space-y-4">
                                            <div>
                                                <h3 className="text-xl sm:text-2xl font-black text-white uppercase tracking-tight leading-tight">{globalResults.seriesMetadata.name}</h3>
                                            ...
                                            <p className="text-xs sm:text-sm text-white/40 leading-relaxed line-clamp-4 sm:line-clamp-3">{globalResults.seriesMetadata.overview}</p>
