...
export function LiveTVRow() {
    ...
    useEffect(() => {
        const fetchTrending = async () => {
            try {
                const res = await fetch('http://localhost:3000/api/iptv/channels?limit=15');
    ...
    return (
        <section className="py-8 px-6 md:px-12 bg-gradient-to-b from-black/0 to-cyan-500/5">
            <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex gap-2">
                    <button
                        onClick={() => scroll('left')}
                        className="p-2 bg-white/5 border border-white/10 rounded-full text-white hover:bg-white/10 hover:border-cyan-500/50 transition-all"
                    >
                        <ChevronLeft size={20} />
                    </button>
                    <button
                    ...
                    <Link
                        to="/tv"
                        className="ml-4 px-4 py-2 bg-cyan-500/10 border border-cyan-500/30 rounded-xl text-cyan-400 text-xs font-bold hover:bg-cyan-500 hover:text-black transition-all flex items-center gap-2"
                    >
                        Ver Todos
            ...
            >
                {loading ? (
                    Array(8).fill(0).map((_, i) => (
                        <div key={i} className="min-w-[200px] h-32 bg-white/5 rounded-2xl animate-pulse" />
                    ))
                ) : (
                    channels.map((channel) => (
                        ...
                        >
                            <Link to="/tv" className="block">
                                ...
                                <h3 className="mt-3 text-xs font-bold text-white/80 group-hover:text-cyan-400 transition-colors uppercase tracking-widest truncate">
                                    {channel.name}
                                </h3>
