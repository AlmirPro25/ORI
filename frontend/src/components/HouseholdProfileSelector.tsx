...
export const HouseholdProfileSelector: React.FC = () => {
    ...
    return (
        <div className="px-4 sm:px-6 md:px-16 pt-6 sm:pt-8 space-y-4">
                ...
                {OPTIONS.map((option) => {
                    ...
                    return (
                        <button
                            ...
                            className={`group flex items-center gap-3 rounded-2xl border px-3.5 sm:px-4 py-3 backdrop-blur-xl transition-all min-w-0 ${
                                active
                                    ? 'border-primary/50 bg-primary/15 text-white shadow-[0_0_30px_-12px_rgba(56,189,248,0.8)]'
                                    : 'border-white/10 bg-white/5 text-white/60 hover:border-white/20 hover:bg-white/10'
                            }`}
                        >
                            <span className={`${active ? 'text-primary' : 'text-white/50'} shrink-0`}>{option.icon}</span>
                            <span className="text-left min-w-0">
                                <span className="block text-[10px] sm:text-[11px] font-black uppercase tracking-[0.14em] sm:tracking-[0.2em] truncate">
                                    {option.label} {locked ? <Lock size={11} className="inline ml-1" /> : null}
                                </span>
                                <span className="block text-[9px] sm:text-[10px] text-white/40 leading-snug">{option.hint}</span>
                            </span>
                        </button>
                    );
                })}
            ...
            {pendingProfile === 'adult' && (
                <div className="max-w-md rounded-[1.5rem] sm:rounded-3xl border border-white/10 bg-white/5 p-4 sm:p-5 backdrop-blur-xl">
                    ...
                    <div className="flex flex-col sm:flex-row gap-3">
                        ...
                        <button
                            onClick={unlockAdult}
                            className="rounded-2xl bg-primary px-5 py-3 text-xs font-black uppercase tracking-[0.18em] sm:tracking-[0.2em] text-black transition hover:bg-primary/90"
                        >
                            Liberar
                        </button>
                    </div>
                    ...
                    <div className="mt-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                        <p className="text-[10px] uppercase tracking-[0.14em] sm:tracking-[0.2em] text-white/35">PIN padrão local: 1234</p>
