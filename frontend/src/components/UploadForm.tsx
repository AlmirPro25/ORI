...
export const UploadForm: React.FC<{ onUploadSuccess?: () => void }> = ({ onUploadSuccess }) => {
    ...
    return (
        <Card className="glass-card border-white/5 overflow-hidden rounded-[1.8rem] sm:rounded-[2.4rem] md:rounded-[3rem] shadow-2xl relative">
            <div className="absolute top-0 right-0 w-32 h-32 bg-primary/10 blur-[80px] -z-10" />

            <CardHeader className="bg-white/5 border-b border-white/5 p-5 sm:p-7 md:p-10">
                <div className="flex items-start sm:items-center gap-3 sm:gap-4 mb-2">
                    <div className="p-3 bg-primary/10 rounded-2xl border border-primary/20 backdrop-blur-3xl shrink-0">
                        <UploadCloud className="text-primary w-6 h-6 animate-pulse" />
                    </div>
                    <div className="min-w-0">
                        <CardTitle className="text-lg sm:text-xl md:text-2xl font-black uppercase italic tracking-tight leading-tight">
                            Asset <span className="text-gradient-primary">Ingestion</span>
                        </CardTitle>
                        <CardDescription className="text-[9px] sm:text-[10px] font-black uppercase tracking-[0.18em] sm:tracking-[0.3em] text-white/30 mt-1">Configure Forge Processing Pipeline</CardDescription>
                    </div>
                </div>
            </CardHeader>

            <CardContent className="p-5 sm:p-7 md:p-10">
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-7 sm:space-y-10">
                    <div className="space-y-6 sm:space-y-8">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-8">
                            <div className="space-y-3">
                                ...
                                <Input
                                    ...
                                    className="h-12 sm:h-14 bg-white/5 border-white/10 rounded-2xl focus:ring-primary/40 focus:border-primary/40 text-sm font-medium placeholder:text-white/10"
                                />
                            ...
                            <div className="space-y-3">
                                ...
                                <select
                                    {...register('category')}
                                    className="w-full h-12 sm:h-14 bg-white/5 border border-white/10 rounded-2xl px-4 text-sm font-medium focus:ring-1 focus:ring-primary/40 outline-none hover:bg-white/10 transition-colors appearance-none text-white italic"
                                >
                        ...
                        <div className="space-y-3">
                            ...
                            <Textarea
                                ...
                                className="bg-white/5 border-white/10 rounded-2xl min-h-[110px] sm:min-h-[120px] focus:ring-primary/40 focus:border-primary/40 text-sm font-medium placeholder:text-white/10 p-4"
                            />
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-8">
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="relative group">
                                    ...
                                    />
                                    <div className="h-28 sm:h-32 border border-dashed border-white/10 rounded-2xl flex flex-col items-center justify-center gap-3 group-hover:border-primary/50 group-hover:bg-primary/5 transition-all duration-500 bg-white/5">
                                        <div className={cn("p-2 rounded-lg transition-colors", watchedFiles ? "bg-primary text-black" : "bg-white/5 text-white/20")}>
                                            <FileVideo size={20} />
                                        </div>
                                        <span className="text-[8px] sm:text-[9px] font-black uppercase tracking-[0.14em] sm:tracking-[0.2em] text-center px-4 truncate max-w-full italic">
                    ...
                    <AnimatePresence>
                        {uploading && (
                            <motion.div
                                ...
                                className="space-y-4 p-4 sm:p-6 md:p-8 bg-black/40 rounded-[1.5rem] sm:rounded-3xl border border-primary/20 backdrop-blur-3xl shadow-glow-sm"
                            >
                                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 text-[10px] font-black uppercase tracking-[0.18em] sm:tracking-[0.3em] text-primary">
                        ...
                        {success && (
                            <motion.div
                                ...
                                className="p-4 sm:p-6 bg-green-500/5 border border-green-500/20 rounded-2xl flex items-center gap-3 sm:gap-4 text-green-500 text-[9px] sm:text-[10px] font-black uppercase tracking-[0.14em] sm:tracking-[0.2em]"
                            >
                        ...
                        {uploadError && (
                            <motion.div
                                initial={{ opacity: 0, y: 10 }}
                                animate={{ opacity: 1, y: 0 }}
                                className="p-4 sm:p-6 bg-red-500/5 border border-red-500/20 rounded-2xl flex items-center gap-3 sm:gap-4 text-red-500 text-[9px] sm:text-[10px] font-black uppercase tracking-[0.14em] sm:tracking-[0.2em]"
                            >
                    ...
                    <Button
                        type="submit"
                        disabled={uploading}
                        className="w-full h-14 sm:h-16 md:h-20 rounded-[1.3rem] sm:rounded-[1.6rem] md:rounded-[1.8rem] text-[10px] sm:text-xs font-black uppercase tracking-[0.18em] sm:tracking-[0.4em] italic bg-white text-black hover:bg-primary transition-all shadow-2xl active:scale-95 group relative overflow-hidden"
                    >
