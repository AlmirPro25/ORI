...
export const IntelligentRecommendations: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      ...
      const profileRes = await axios.get('http://localhost:3000/api/intelligence/profile', {
        headers: { Authorization: `Bearer ${token}` },
      });
      setProfile(profileRes.data);

      // Carregar recomendações
      const recsRes = await axios.get('http://localhost:3000/api/intelligence/recommendations', {
          ...
          {recommendations.map((video) => (
            ...
            >
              ...
              <div className="relative aspect-video bg-gray-900">
                {video.thumbnailPath ? (
                  <img
                    src={`http://localhost:3000/uploads/${video.thumbnailPath}`}
                    alt={video.title}
