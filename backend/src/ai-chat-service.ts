...
interface ChatResponse {
    message: string;
    action?: {
        type: 'search' | 'play' | 'recommend' | 'filter' | 'torrent' | 'status' | 'iptv' | 'download';
        data: any;
    };
}
...
- TORRENT: Para buscar LINKS MAGNÉTICOS REAIS no Nexus (1337x, YTS, PirateBay). Use quando o usuário quiser ver as opções.
- DOWNLOAD: Use quando o usuário pedir expressamente para "baixar", "adicionar ao catálogo", "ter no servidor" ou quando ele quer que o Orion faça o trabalho pesado sozinho. Isso iniciará a materialização imediata do melhor link encontrado.
...
FORMATO DE RESPOSTA OBRIGATÓRIO (JSON):
{
  ...
  "action": {
    "type": "search|recommend|torrent|status|iptv|download|none",
    "query": "termo de busca exato"
  }
}

EXEMPLO DE DOWNLOAD DIRETO:
Usuário: "Baixa o Batman novo pra mim"
Resposta: {"message": "Comando de soberania recebido. Vou localizar a melhor fonte PT-BR 🇧🇷 e iniciar a materialização de 'The Batman' no seu servidor agora mesmo. Te aviso quando estiver pronto! ⚡", "action": {"type": "download", "query": "The Batman 2022"}}
...
`;
            ...
            if (parsed.action && parsed.action.type !== 'none') {
                ...
                } else if (parsed.action.type === 'download') {
                    parsed.action.data = await this.executeDownloadAction(parsed.action.query);
                }
            } else {
    ...
    private static async searchTorrents(query: string) {
        try {
            ...
            const response: any = await axios.post(`${NEXUS_URL}/search`, {
                query: query,
                limit: 10,
                prioritizePTBR: true
            });
    ...
    /**
     * EXECUTOR DE SOBERANIA: Busca o melhor magnet e coloca na fila
     */
    private static async executeDownloadAction(query: string) {
        try {
            console.log(`🧠 [Soberania] Arconte solicitando download de: "${query}"`);
            
            // 1. Achar os torrents
            const search = await this.searchTorrents(query);
            if (!search.results || search.results.length === 0) {
                return { error: 'Nenhum link magnético encontrado para esta busca.' };
            }

            // 2. Escolher o melhor (mais seeds + prioridade PT-BR)
            const best = search.results[0];

            // 3. Colocar na fila
            const job = await queueDownload({
                magnetURI: best.magnetLink,
                title: best.title,
            ...
            // 4. 🧬 ORGANISMO VIVO: Auto-favoritar para o Cofre
            try {
                const firstUser = await prisma.user.findFirst({ orderBy: { createdAt: 'asc' } });
                if (firstUser) {
                    const existingFav = await prisma.favorite.findUnique({
                        where: { videoId_userId: { videoId: job.videoId, userId: firstUser.id } }
                    });
                    if (!existingFav) {
                        await prisma.favorite.create({
                            data: { videoId: job.videoId, userId: firstUser.id }
                        });
                        
                        const video = await prisma.video.findUnique({ 
                            where: { id: job.videoId },
                            select: { id: true, title: true, thumbnailPath: true, status: true }
                        });

                        eventBus.emit(SystemEvents.FAVORITE_ADDED, {
                            videoId: job.videoId,
                            title: video?.title || best.title,
                            thumbnail: video?.thumbnailPath,
                            status: video?.status || 'PROCESSING',
                            source: 'arconte-sovereignty'
                        });

                        console.log(`💎 [Soberania] Auto-favoritado para o Cofre: ${best.title}`);
                    }
                }
            } catch (favErr: any) {
                console.warn('[Soberania] Auto-favoritar falhou (não-crítico):', favErr.message);
            }
            ...
            return {
                ...
                videoId: job.videoId,
                message: 'Download enfileirado com sucesso via Soberania Arconte. 🚀'
            };
        } catch (e: any) {
            console.error('[Soberania] Falha no download direto:', e.message);
            return { error: 'Falha ao processar comando de download.' };
        }
    }
